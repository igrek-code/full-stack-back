...
public class ProductController {
  ...
  public ProductDto updateProduct(
      @ApiParam(value = "Product id to modify", required = true) @PathVariable(required = true)
          long id,
      @ApiParam(value = "New product information", required = true) @Valid @RequestBody
          ProductDto productDto) {
    Product product = modelMapper.map(productDto, Product.class);
  ...
  public List<ProductDto> getProductsByShopId(
      @ApiParam(value = "Store id", required = true)
          @RequestParam(defaultValue = "0", value = "shopId")
          long shopId) {
    return productService.findAllByShopId(shopId).stream()
